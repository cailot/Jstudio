...
//		cal();
	...
	public void cal()
	{
		...
		window.getDecorView().getWindowVisibleDisplayFrame(rectgle); 
		// status bar
		int statusBarTop = rectgle.top; 
		// contents
		int contentViewTop = window.findViewById(Window.ID_ANDROID_CONTENT).getTop();
		int contentViewHeight = window.findViewById(Window.ID_ANDROID_CONTENT).getHeight();
		// title bar
		int titleBarTop = contentViewTop - statusBarTop;
		int titleBarHeight = contentViewHeight - statusBarTop;
		Log.i(M7k37Constants.HEADER, "whole= " + wholeHeight +
				" , statsBarTop = " + statusBarTop +
		...
		//wholeHeight = wholeHeight - (int)(actionBarHeight) ;
		wholeHeight = wholeHeight - contentViewTop ;
		itemHeight = (int)(wholeHeight/size);
